        ...
        if (MyServer) {
            ...
            if (MyEconomy) {
                if (MyUser) {
                    ...
                    let Axe = MyUser.inv.axe;
                    if (Axe < 1) return message.reply("You need an axe before you can use this command!" + " " + `\`${MyServer.prefix}shop\``)

                    let MinPay = EconomyStorage.pays.lumberPay.min;
                    let MaxPay = EconomyStorage.pays.lumberPay.max;

                    let pay = generate.ranInt(MinPay, MaxPay);
                    let woodAmount = generate.ranInt(1, 19);

                    const response = new MessageEmbed({
                        description: `${message.author} you have chopped down **${woodAmount} Tree's** and earned **${MyEconomy.currency} ${pay}**`,
                        color: "GREEN"
                    });
